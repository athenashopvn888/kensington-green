...
const store = {
  ...
  displayName: "Kensington Green",
  deliveryPhone: "+16473651957",
} as const;
    ...
    <div className="product-pricing">
      ...
      {(regular28 || member || bundles.length > 0) && <div className="decision-prices">
        ...
        {bundles.map((offer, index) => {
          ...
          const each = get28gBundleEachDisplayPrice(quantity, total, offer.perUnitPrice);
          return <div className="decision-tile bundle-decision" key={`${offer.kind}-${quantity}-${index}`}><span>{quantity} × 28g DEAL</span><div className="bundle-numbers"><strong>{formatCurrency(each)} <small>each</small></strong><b>{formatCurrency(total)} <small>total</small></b></div></div>;
        })}
        {regular28 && <div className="decision-tile standard-28"><span>STANDARD 28g</span><strong>{formatCurrency(regular28.price)}</strong><small>Regular price</small></div>}
      </div>}
    </div>
  );
}
...
export default function Catalog() {
  ...
  useEffect(() => {
    ...
    fetch(`https://milestone-1-demo.vercel.app/api/catalog?store=${store.id}`, { signal: controller.signal })
    ...
    <div className="qlc-original-shell">
      <header className="store-header qlc-store-header">
        <div className="header-main">
          <Link className="brand-lockup" href="/" aria-label={`${store.name} homepage`}><span className="brand-mark">{store.shortName}</span><span><strong>{store.displayName}</strong><small>Delivery menu</small></span></Link>
      ...
      <main className="delivery-page" id="top">
        ...
        <section className="menu-layout qlc-three-column-layout" id="menu">
          ...
          <section className="menu-main qlc-menu-main">
            <div className="menu-tools"><div><p className="eyebrow">{store.shortName} FLOWER MENU</p><h2>Flowers</h2></div><label className="menu-search"><span>Search</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Product or category" /></label></div>
            ...
            <div className="product-grid qlc-list-grid">
              {filtered.map((product) => {
                ...
                return <article className="product-card qlc-horizontal-card" key={product.publicProductId}>
                  <button className="product-image-button" type="button" onClick={() => setSelectedProduct(product)} aria-label={`View details for ${product.name}`}>{product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on the ${store.name} delivery menu`} fill sizes="(max-width: 640px) 50vw, 104px" unoptimized /> : <span>No image</span>}</button>
        ...
        <section className="qlc-order-steps" id="how-to-order" tabIndex={-1} aria-labelledby="how-to-order-title">
          ...
          <h2 id="how-to-order-title">LIVE ORDER connects you with the {store.shortName} dispatcher.</h2>
          <ol>
      ...
      <ProductDetailsDrawer product={selectedProduct} storeName={store.displayName} onClose={closeDetails} pricing={selectedProduct ? <ProductPricing product={selectedProduct} /> : null} />
    </div>
  );
}
