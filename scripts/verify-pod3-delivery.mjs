...
const page = await read("app/weed-delivery-toronto/page.tsx");
...
assert.deepEqual(menu.store, { id: "KGR", code: "KSC01", name: "Kensington Green", pod: "POD03" });
...
assert(catalog.includes("api/catalog?store=") && catalog.includes('id: "KGR"'));
...
assert(chat.includes('STORE_ID = "KGR"') && chat.includes('"NEW_CUSTOMER"') && chat.includes('"RETURNING_CUSTOMER"'));
...
assert(chat.includes("/api/web-chat/phone") && chat.includes("phoneConfirmation: replacementPhoneConfirmation") && chat.includes("phoneVersion: conversation.phoneVersion"));
assert(chat.includes("START ANOTHER ORDER") && chat.includes("/api/web-chat/order-cycle") && chat.includes("requestId: crypto.randomUUID()"));
assert(chat.includes("securely retained for future identity and address verification"));
...
assert(page.includes("Dundas West Cannabis Delivery") && !page.includes("Coming Soon"));
...
assert(navbar.includes("STORE MENU") && navbar.includes("Weed Delivery"));
assert(home.includes("STORE MENU") && home.includes("Explore Weed Delivery"));
assert(footer.includes('<Link href="/weed-delivery-toronto">Neighbourhood Delivery</Link>') && !footer.includes("Delivery (Coming Soon)"));
...
console.log(JSON.stringify({ storeId: "KGR", podId: "POD03", products: 63, descriptions: 58, images: 63, root }, null, 2));
